...
FLIPBOOK.Book3 = function(t, e, i) {
 ...
 e.on("toolMove", function() {
  ...
  if (isNaN(s.scroll.scale)) s.zoomTo(i.zoomMin);
   ...
   var t = i.main.zoom;
   s.scroll.options.eventPassthrough = 1 < t ? "" : "vertical", s.scroll.options.freeScroll = 1 < t, s.scroll.refresh()
  }
 }), this.scroll.on("zoomStart", function() {});
 var o = this.wrapper.style;
 o.width = String(2 * this.pageWidth) + "px", o.height = String(this.pageHeight) + "px";
 ...
 for (var a = 0; a < i.pages.length; a++)(n = new FLIPBOOK.Page3(this, a, i.pages[a].src, i.pages[a].htmlContent)).hideHtml(), this.pagesArr.push(n), this.$centerContainer.append(n.$wrapper), this.flippedright++;
...
}, FLIPBOOK.Book3.prototype.onZoom = function() {}, FLIPBOOK.Book3.prototype.getNumPages = function() {}, FLIPBOOK.Book3.prototype.zoomTo = function(t, e, i, s) {
 if (this.enabled) {
  i = i || 0, s = s || 0, e = e || 0;
  var o = this.scroll;
 ...
 if (this.centerContainerPosition != t) {
  ...
  if (e) {
   ...
   i = i || 0, jQuery({
    ...
    step: function(t) {
     o._setStyle(o.centerContainerStyle, FLIPBOOK.IScroll.utils.style.transform, "translateX(" + t + "px) translateZ(0)")
    },
    complete: function() {
 ...
 void 0 === t && (t = !0);
 for (var e = this, i = FLIPBOOK.IScroll.utils.style.transform, s = 0; s < this.pagesArr.length; s++) s == this.flippedleft ? this.pagesArr[s].show() : s != this.flippedleft - 1 || this.options.singlePageMode ? this.pagesArr[s].hide() : this.pagesArr[s].show(), this.pagesArr[s]._setZIndex(0), this.pagesArr[s]._setStyle(this.pagesArr[s].wrapper.style, i, "");
 ...
 n ? (e.options.main.setLoadingProgress(.1), n.load(function() {
  e.options.main.setLoadingProgress(1), n.showHtml(), o ? o.load(function() {
   e.options.main.setLoadingProgress(1), r && r.load(), p && p.load(), h && h.load(), a && a.load(), o.showHtml()
  }) : (r && r.load(), p && p.load())
 })) : (e.options.main.setLoadingProgress(.1), o.load(function() {
  e.options.main.setLoadingProgress(1), h && h.load(), a && a.load(), o.showHtml()
 }))
 ...
 if (this.enabled && !this.flipping)
  if ((this.options.singlePageMode || t % 2 != 0) && t--, t < 0 && (t = 0), t > this.pagesArr.length && (t = this.pagesArr.length), t != this.rightIndex) {
   ...
   this.options.singlePageMode || (1 == this.view ? t < this.rightIndex ? this.focusRight(s) : this.focusLeft(s) : 0 == t ? this.focusRight(s) : t == this.pagesArr.length ? this.focusLeft(s) : this.focusBoth(s));
   var o = this;
   (this.goingToPage = t) > this.rightIndex ? (end = 180, (o.angle <= 0 || 180 <= o.angle || !o.angle) && (o.angle = 1)) : t < this.rightIndex && (end = -180, (0 <= o.angle || o.angle <= -180 || !o.angle) && (o.angle = -1)), this.options.singlePageMode && (s /= 2), jQuery({
    someValue: o.angle
   }).animate({
    someValue: end
   }, {
    ...
    complete: function() {
     o.setRightIndex(t), o.flippedleft = t, o.flippedright = o.pagesArr.length - t, o.updateVisiblePages(), o.angle = 0, o.flipping = !1, o.options.main.turnPageComplete()
    }
 ...
 if (!this.isZoomed() && "flipbook-page-link" != t.target.className && "start" != e && "up" != i && "down" != i && !this.flipping) {
  ...
  if ("cancel" == e && n <= 1 && (0 < a ? this.nextPage() : a < 0 && this.prevPage()), "end" == e && n <= 1 && (0 < a ? this.nextPage() : a < 0 && this.prevPage()), "move" == e && n <= 1) {
   if (0 < a) {
   ...
   this.goingToPage != this.rightIndex && 0 <= this.goingToPage && this.goingToPage <= this.pagesArr.length && (this.options.instantFlip || this._setPageAngle(a))
  }
 }
 ...
 var e, i, s, o;
 if (this.angle = t, this.hideHtml(), this.options.singlePageMode) 0 < t ? ((s = this.pagesArr[this.rightIndex])._setAngle(t / 2), (i = this.pagesArr[this.goingToPage]) && i.show()) : ((o = this.pagesArr[this.goingToPage]).show(), o._setAngle(t / 2 + 90), s = this.pagesArr[this.rightIndex]);
 else if (0 < t) {
  if (1 == this.view && this.isFocusedLeft()) return;
  s = this.pagesArr[this.rightIndex], o = this.pagesArr[this.goingToPage - 1], this.applyAngles(s, o, t, 90, 0), i = this.pagesArr[o.index + 1], s.setShadowOpacity((t - 10) / 600), o.setShadowOpacity((180 - t - 10) / 600), i && i.show()
 } else {
  if (1 == this.view && this.isFocusedRight()) return;
  o = this.pagesArr[this.rightIndex - 1], s = this.pagesArr[this.goingToPage], this.applyAngles(s, o, t, -90, 180), s.setShadowOpacity((180 + t - 10) / 600), o.setShadowOpacity((-t - 10) / 600), (e = this.pagesArr[this.goingToPage - 1]) && e.show()
 }
 ...
 i < s ? (t && (t.show(), t._setAngle(i + o)), e && (e.hide(), e._setAngle(45))) : (e && (e.show(), e._setAngle(i + o)), t && (t.hide(), t._setAngle(45)))
 ...
 this.options.responsiveView && e <= this.options.responsiveViewTreshold && a < 2 * r ? (this.view = 1, n / h < e / i ? g() : d()) : (this.view = 2, s / o <= e / i ? g() : d()), this.updateBookPosition()
 ...
 this.book = t, this.options = t.options, this.texture = i, this.html = s, this.index = e, this.$wrapper = jQuery("<div>").addClass("flipbook-page3").width(t.options.pageWidth).height(t.options.pageHeight), this.wrapper = this.$wrapper[0], this.htmlContentScale = this.book.$wrapper.height() / 1e3;
 var o = t.options;
 o.pagePreloader ? this.$preloader = jQuery('<img src="' + o.pagePreloader + '" class="flipbook-page-preloader-image">').appendTo(this.$wrapper) : this.$preloader = jQuery('<img src="' + o.assets.spinner + '" class="flipbook-page-preloader">').appendTo(this.$wrapper), this.$image = jQuery("<div>").appendTo(this.$wrapper).addClass("flipbook-page3-image"), this.options.doublePage && this.$image.css("overflow", "hidden"), this.$canvas = jQuery("<div>").appendTo(this.$wrapper).addClass("flipbook-page3-html"), this.$shadow = jQuery("<div>").appendTo(this.$wrapper).addClass("flipbook-page3-shadow"), this.shadow = this.$shadow[0], this.hidden = !1, this.hide(), this.zIndex = 0, this.options.singlePageMode ? (this.$wrapper.addClass("flipbook-page3-front"), this.type = "front") : e % 2 == 0 ? (this.$wrapper.css("left", String(this.book.options.pageWidth - 1) + "px").addClass("flipbook-page3-front"), this.type = "front") : (this.$wrapper.addClass("flipbook-page3-back"), this.type = "back")
}, FLIPBOOK.Page3.prototype = {
 load: function(n) {
  if (1 != this.loaded) {
   this.loaded = !0;
   var h = this,
    t = h.options.rightToLeft ? this.book.pagesArr.length - this.index - 1 : this.index;
   h.options.pdfMode ? this.options.main.loadPage(t, this.options.pageTextureSize, function() {
    n && n.call(h)
    ...
    "left" == h.options.pages[t.index].side ? scaleX = 2 : "right" == h.options.pages[t.index].side ? (s = "-50% ", scaleX = 2) : scaleX = 1, jQuery(e).appendTo(h.$image).css(FLIPBOOK.IScroll.utils.style.transform, "scaleX(" + scaleX + ") translateX(" + s + ") translateZ(0)");
    var o = t.htmlContent;
    !o || h.options.doublePage && 0 != h.index && h.index % 2 == 0 || (h.$html = jQuery(o).appendTo(h.$canvas), h.$html[0].style.transform = "scale(" + h.htmlContentScale + ") translateZ(0)"), h.hidden && h.hideHtml(), h.$wrapper.css("background", "none"), n && n.call(h)
   })
  } else n && n.call(this)
 },
 unload: function() {
 ...
 setMat: function(t, e, i) {
  this.mat || (this.mat = !0, this.$canvas.empty(), jQuery(t).appendTo(this.$canvas), this.$preloader.hide(), this.$wrapper.css("background", "none"), !e || this.options.doublePage && 0 != this.index && this.index % 2 == 0 || (this.$html = jQuery(e).appendTo(this.$canvas), this.$html[0].style.transform = "scale(" + this.htmlContentScale + ") translateZ(0)"), this.hidden && this.hideHtml())
 },
 hideHtml: function() {
 ...
 _setAngle: function(t) {
  if (this.angle != t) {
   this.angle = t;
   var e, i, s = FLIPBOOK.IScroll.utils.style.transform,
    o = "translateZ(0)";
   "3d" == this.book.options.viewMode ? e = "front" == this.type ? "rotateY(" + String(-t) + "deg) " + o : "rotateY(" + String(180 - t) + "deg) " + o : "2d" == this.book.options.viewMode && ("front" == this.type ? (90 < t && (t = 90), e = "scaleX(" + String((180 - 2 * t) / 180) + ") " + o) : (t < 90 && (t = 90), e = "scaleX(" + String(2 * t / 180 - 1) + ") " + o)), this._setStyle(this.wrapper.style, s, e);
   var n = 0;
